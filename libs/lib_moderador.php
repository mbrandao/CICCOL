    ...
    case "moderador":
        switch ($request_action) {
            ...
            case "inserir_moderador":
                ...
                $SQL = pg_query("SELECT id_user FROM docente WHERE matricula = '$request_matricula'") or die("Não foi possível fazer a consulta na tabela docente".pg_last_error());
            ...
            case "update_moderador":
                ...
                $SQL = "SELECT id_user FROM docente WHERE id_user = '$request_id'";
              ...
              if($result == ""){
                   ...
                   $SQL2 = pg_query("UPDATE autenticacao SET identificador = '$request_cpf' WHERE (id_user = $request_id AND id_tpuser = '2')") or die("Nao pode atualizar autenticacao".pg_last_error());
                   ...
                   $SQL3 = pg_query("UPDATE autenticacao SET identificador = '$request_cpf' WHERE (id_user = $request_id)") or die("Nao pode atualizar autenticacao".pg_last_error());
            ...
            case "apagar_moderador":
                ...
                //Verifica se está cadastrado na tabela docente
                $SQL = pg_query("SELECT id_user FROM docente WHERE id_user = '$request_id'") or die("Não foi possível fazer a consulta na tabela docente".pg_last_error());
                $row = pg_fetch_array($SQL);

                if($row == ""){
                    //Apaga da tabela usuario e adminmoderador
                    $SQL1 = pg_query("DELETE FROM adminmoderador WHERE (id_user = $request_id)");
            ...
            case "buscar_moderador":
                ...
                elseif($retorno == '2'){
                    $SQL = pg_query("select U.nome, U.cpf, U.sobrenome, U.email, U.dt_nasc FROM usuario U JOIN docente D on D.matricula = '$row4[matricula]' AND U.id_user = D.id_user") or die ("Não foi possível consultar a tebela usuario".pg_last_error());
                    $row = pg_fetch_array($SQL);
